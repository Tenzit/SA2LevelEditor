...
std::list<TexturedModel*> KEYDOOR::models;
CollisionModel* KEYDOOR::cmBase;
...
KEYDOOR::KEYDOOR(char data[32], bool useDefaultValues)
{
    ...
    collideModelOriginal = KEYDOOR::cmBase;
    collideModelTransformed = KEYDOOR::cmBase->duplicateMe();
...
std::list<TexturedModel*>* KEYDOOR::getModels()
{
    return &KEYDOOR::models;
}

void KEYDOOR::loadStaticModels()
{
    if (KEYDOOR::models.size() > 0)
    {
        return;
    }

#ifdef DEV_MODE
    std::fprintf(stdout, "Loading KEYDOOR static models...\n");
#endif
    loadObjModelFromPCGame(&KEYDOOR::models, (struct PCMeshObject *)0xabbbf0);
    KEYDOOR::cmBase = loadCollisionModelFromPCGame((struct PCMeshObject *)0xabbbf0);
}

void KEYDOOR::deleteStaticModels()
{
#ifdef DEV_MODE
    std::fprintf(stdout, "Deleting KEYDOOR static models...\n");
#endif
    Entity::deleteModels(&KEYDOOR::models);
    Entity::deleteCollisionModel(&KEYDOOR::cmBase);
}

void KEYDOOR::updateValue(int btnIndex)
{
    char buf[128];
...
void KEYDOOR::updateEditorWindows()
{
    ...
    SetWindowTextA(Global::windowValues[1], "KEYDOOR");
    SetWindowTextA(Global::windowValues[2], std::to_string(position.x).c_str());
    SetWindowTextA(Global::windowValues[3], std::to_string(position.y).c_str());
    SetWindowTextA(Global::windowValues[4], std::to_string(position.z).c_str());
    SetWindowTextA(Global::windowValues[5], std::to_string(rotationX).c_str());
...
void KEYDOOR::fillData(char data[32])
{
    data[1] = (char)ID;
